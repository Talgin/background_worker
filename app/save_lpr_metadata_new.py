...
logging.info(f"Starting LPR worker...")
# Init Kafka consumer
...
def save_crops(content_id, frame, cars, file_type):
    ...
    out_path = os.path.join(Config.OUTPUT_PATH_LPR, content_id)
    ...
    # crop_0_1.jpg
    # crop_1_0.jpg
    frame_name = file_type + '_' + str(frame_cntr) + '.jpg'
    # if file_type == 'video': 
    ...
    for car in cars:
        license_plate = car.get('number')
        logging.info(f"License plate': {license_plate}")
        x, y, width, height = car.get('x'), car.get('y'), car.get('width'), car.get('height')
        coordinates = [x,y,width,height]
        if frame_width < width or frame_height < height: 
            logging.info('******************** Incorrect car coordinates! ********************')
            logging.info(f'frame width - {frame_width}, frame height - {frame_height}')
            logging.info(f'plate coordinates: x - {x}, y - {y}, width - {width}, height - {height}')
            continue
        ...
        cropped_image = image.crop((int(x), int(y), int(x + width), int(y + height)))
        cropped_image.save(os.path.join(out_path, crop_name))

        output_cars.append({'crop': crop_name, 'license_plate': license_plate, 'coordinates': coordinates})
        crop_cntr += 1
...
def consume_and_save():
    try:
        ...
        while True:
            ...
            # mongo_incidents_collection.insert_one(output_data)
            mongo_incidents_collection.update_one({"_id": ObjectId(content_id)},{"$set":{"analytics_result": [output_data]}})
                
            logging.info('Message saved!')
            logging.info(f'Content_id: {content_id}  \n')
            
    except KafkaException as e:
        logging.info(f'Kafka error') # print(f"[KafkaException]: {e.strerror}, filename: {e.filename}")
    finally: 
